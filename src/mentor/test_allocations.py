# ============================================================
# DOMINIC HERE: THIS FILE IS ALMOST IDENTICAL TO THE OTHER ONE
# BUT HAS BEEN SLIGHTLY MODIFIED TO
# TAKE IN AN INTEGER TO RUN WITH THE 10 RANDOMIZED ALLOCATIONS
#
# put simply, it was created to test different allocations
# ============================================================
# Inputs: N assets
#          -expected return
#          -standard deviation
...
def main(x: int):
    ...
    file_path = Path('../../data/mentor/Returns_Std_Devs.csv')
    df_Returns_Std_Deviations = pd.read_csv(file_path)

    file_path2 = Path('../../data/mentor/Index_Correlations.csv')
    df_Index_Correlations = pd.read_csv(file_path2)

    file_path3 = Path(f'../../data/test_allocations/balance_allocation_{x}.csv')
    ...
    correlation_columns = np.array(df_Index_Correlations[['vfiax','vexax','vgslx','vdipx','vemax','vgpmx','vbtlx','vaipx','vusxx','cash']]).astype(float)
